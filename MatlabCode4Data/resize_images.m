...
%                           Photogrammetric Engineering and Remote Sensing, vol. 63, no. 6, pp. 691ï¿?699, June 1997.
...
%                           Photogrammetric Engineering and Remote Sensing, vol. 72, no. 5, pp. 591ï¿?596, May 2006.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [I_MS_LR, I_PAN_LR] = resize_images(I_MS,I_PAN,ratio,sensor,MTF)
...
switch sensor
    ...
    case 'none'
        ...
        flag_resize_new = 2; % Bicubic Interpolator
end
...
%     I_MS_LP = zeros(round(size(I_MS,1)/ratio),round(size(I_MS,2)/ratio),size(I_MS,3));
    I_MS_LP = zeros(floor(size(I_MS,1)/ratio),floor(size(I_MS,2)/ratio),size(I_MS,3));
    
    for idim=1:size(I_MS,3)
%         I_MS_LP(:,:,idim) = imresize(I_MS(:,:,idim),1/ratio);
        I_MS_LP(:,:,idim) = imresize(I_MS(:,:,idim),[floor(size(I_MS,1)/ratio),floor(size(I_MS,2)/ratio)]);
    end
...
%     I_PAN_LR = imresize(I_PAN,1/ratio);
    I_PAN_LR = imresize(I_PAN,[floor(size(I_PAN,1)/ratio), floor(size(I_PAN,2)/ratio)]);
...
%         I_PAN_LR = imresize(I_PAN,1/ratio,'nearest');
        I_PAN_LR = imresize(I_PAN,[floor(size(I_PAN,1)/ratio), floor(size(I_PAN,2)/ratio)],'nearest');

    else
%         I_PAN_LR = imresize(I_PAN,1/ratio); % default: bicubic
        I_PAN_LR = imresize(I_PAN,[floor(size(I_PAN,1)/ratio), floor(size(I_PAN,2)/ratio)],'nearest');
...
%     I_MS_LR = imresize(I_MS_LP,1/ratio,'nearest');
    I_MS_LR = imresize(I_MS_LP,[floor(size(I_MS_LP,1)/ratio), floor(size(I_MS_LP,2)/ratio)],'nearest');
end

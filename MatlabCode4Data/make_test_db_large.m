...
% This module can generate test samples with larger scale from original PAN and LRMS images and save it with h5 format.
%
%% remark:
% @test_data_dir : directory of original PAN and LRMS images
...
L = 128; % patch size for large-scale test set
tot_cnt = 200; % total number of samples to generate
kount = tot_cnt;
...
for i = 1:cnt_item
    ...
    create_Data(pan1, mul1, tot_cnt, L, ratio, sensor, countEntered);
    countEntered = countEntered + kount(i);
end
sample_index = 1:sum(kount);
tst_index = datasample(sample_index, tot_cnt,'Replace',false);

fname = sprintf('%s%slarge_tst(%s)_%04d-%04d.h5', save_path, filesep, sensor, L, L);
cur_index = tst_index;
